...
const languageOptions = [
  ...
  { value: 'mandinka', label: 'Mandinka (mandingo / mandingue)', color: '#d9d2e9' },
  ...
  { value: 'pulaarSenegal', label: 'Pulaar du Sénégal (Peul du Sénégal)', color: '#cfe2f3' },
  { value: 'pulaarMali', label: 'Pulaar du Mali (Peul du Mali)', color: '#c9daf8' },
  { value: 'pulaarGuinee', label: 'Pulaar de Guinée (Peul de Guinée)', color: '#d9d2e9' },
...
const sortedLanguageOptions = languageOptions.sort((a, b) => a.label.localeCompare(b.label));

const customStyles = {
  control: (base) => ({
    ...base,
    boxShadow: 'rgba(0, 0, 0, 0.12) 0px 1px 1px 0px, rgba(61, 59, 53, 0.16) 0px 0px 0px 1px, rgba(61, 59, 53, 0.08) 0px 2px 5px 0px',
    borderRadius: '5px',
    border: '0px',
  ...
  option: (base, state) => ({
    ...base,
    backgroundColor: state.isFocused ? state.data.color : state.isSelected ? state.data.color : 'white',
...
function Reservation() {
  ...
  const handleAddLanguage = () => {
    if (selectedLanguage && serviceDate && startTime && endTime && interpretationType) {
      const formattedDate = new Date(serviceDate).toLocaleDateString('fr-FR');
      setLanguages([...languages, { language: selectedLanguage, date: formattedDate, startTime, endTime, interpretationType }]);
  ...
  return (
    <div className="App" style = {{
      ...
      <Container>
        ...
        <Paragraph>Vous accueillez ou accompagnez un public étranger et les barrières linguistiques vous empêchent de vous comprendre et de mener à bien votre mission ? <br />Nous mettons à votre disposition des interprètes expérimentés pour lever les dites barrières.</Paragraph>
        <section className="contact">
          <Subtitle>ET SI ON FAISAIT CONNAISSANCE ?</Subtitle>
          <form>
            <div className="form-group">
              <label htmlFor="youAre">VOUS ÊTES : <span className="required">*</span></label>
            ...
            <div className="form-group">
              <label htmlFor="structureName">NOM DE VOTRE STRUCTURE : <span className="required">*</span></label>
              <input type="text" id="structureName" placeholder="Nom de votre structure" className="input-style"/>
            </div>
            <div className="form-group">
              <label htmlFor="function">FONCTION : <span className="required">*</span></label>
              <input type="text" id="function" placeholder="Fonction" className="input-style"/>
            </div>
            <div className="form-group">
              <label htmlFor="email">E-MAIL : <span className="required">*</span></label>
              <input type="email" id="email" placeholder="E-mail" className="input-style"/>
            </div>
            <div className="form-group">
              <label htmlFor="postalAddress">ADRESSE POSTALE : <span className="required">*</span></label>
              <input type="text" id="postalAddress" placeholder="Adresse postale" className="input-style"/>
            </div>
            <div className="form-group">
              <label htmlFor="phoneNumber">NUMERO DE TELEPHONE : <span className="required">*</span></label>
              <input type="tel" id="phoneNumber" placeholder="Numéro de téléphone" className="input-style"/>
            </div>
            <div className="form-group">
              <label htmlFor="interpretationType">TYPE D'INTERPRÉTARIAT : <span className="required">*</span></label>
              <select id="interpretationType" className="input-style" value={interpretationType} onChange={(e) => setInterpretationType(e.target.value)}>
                ...
                <option value="videoConference">Interprétariat par visio-conférence</option>
              </select>
            </div>
            <div className="form-group">
              <label htmlFor="serviceDate">DATE DE LA PRESTATION : <span className="required">*</span></label>
              <input type="date" id="serviceDate" className="input-style" value={serviceDate} onChange={(e) => setServiceDate(e.target.value)} />
            </div>
            <div className="form-group time-group">
              <div className="time-field">
                <label htmlFor="startTime">HEURE DE DÉBUT DE LA PRESTATION : <span className="required">*</span></label>
                <input type="time" id="startTime" className="input-style" value={startTime} onChange={(e) => setStartTime(e.target.value)}/>
              </div>
              <div className="time-field">
                <label htmlFor="endTime">HEURE DE FIN DE LA PRESTATION : <span className="required">*</span></label>
                <input type="time" id="endTime" className="input-style" value={endTime} onChange={(e) => setEndTime(e.target.value)}/>
              </div>
            </div>
            <div className="form-group">
              <label htmlFor="language">SÉLECTIONNEZ UNE LANGUE : <span className="required">*</span></label>
              <Select
                id="language"
              ...
              />
              <button type="button" onClick={handleAddLanguage} className="add-language-button">Ajouter</button>
            </div>
            <div className="form-group">
              ...
              <div className="reservation-summary">
                ...
                {languages.map((lang, index) => (
                  <div key={index} className="selected-language">
                    ...
                    <span className="column">{lang.startTime} - {lang.endTime}</span>
                    <span className="column">{lang.interpretationType === 'telephone' ? 'Téléphonique' : lang.interpretationType === 'inPerson' ? 'Présentiel' : 'Visio-conférence'}</span>
                  </div>
                ))}
              </div>
            </div>
            <div className="form-group">
              ...
              <textarea id="message" placeholder="Écrivez votre message" className="input-style"></textarea>
            </div>
            <Buttoncontainer>
